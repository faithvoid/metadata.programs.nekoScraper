...
parentheses_enabled    = addon.getSetting('parentheses_enabled')
        ...
        for k in raw:
            for f in raw[k]:
                if not f or f[0] == 0:
                    continue
                fid = f[1]
                name = FEATURE_ID_TABLE.get(fid, "Unknown")

                # Build numeric part
                if len(f) == 4:
                    num_part = "{}-{}".format(f[2], f[3])
                elif len(f) == 3:
                    num_part = str(f[2])
                else:
                    num_part = ""

                # Minimal change: wrap in parentheses if enabled
                if parentheses_enabled == "true" and num_part:
                    feature_str = "{} ({})".format(name, num_part)
                else:
                    feature_str = "{} {}".format(name, num_part) if num_part else name

                (g if fid <= 25 else o).append(feature_str)
...
def download_artwork(xmid, folder):
    ...
    try:
        ...
        game_root = os.path.dirname(os.path.dirname(folder))
        dest_icon = os.path.join(folder, "thumb.jpg")
        tbn_path = os.path.join(game_root, "default.tbn")

        shutil.copy(dest_main, dest_icon)
        shutil.copy(dest_main, tbn_path)
...
def download_igdb_screenshots(game_id, token, folder):
    ...
    try:
        ...
        for i, shot in enumerate(result):
            ...
            if not imgid:
                continue
            url = "https://images.igdb.com/igdb/image/upload/t_screenshot_med/%s.jpg" % imgid
            dest = os.path.join(folder, "screenshot-%d.jpg" % (i+1))
            try:
                req = urllib2.Request(url, headers={'User-Agent': 'nekoScraper'})
                resp = urllib2.urlopen(req, timeout=15)
                tmp = dest + ".part"
...
def download_igdb_fanart(game_id, token, folder):
    try:
        if not os.path.exists(folder):
            os.makedirs(folder)
    except:
    ...
    def try_download(url):
        try:
            ...
            log("Fanart download failed: %s -> %s" % (url, e), xbmc.LOGERROR)
            return False

    try:
        q = "fields image_id; where game = %d; limit 1;" % int(game_id)
        req = urllib2.Request("https://api.igdb.com/v4/artworks", q.encode("utf-8"))
        req.add_header("Client-ID", igdb_clientid)
        req.add_header("Authorization", "Bearer %s" % token)
        req.add_header("Accept", "application/json")
        ...
        if isinstance(result, list) and result:
            imgid = result[0].get("image_id")
            if imgid:
                url = "https://images.igdb.com/igdb/image/upload/t_720p/%s.jpg" % imgid
                if try_download(url):
                    return
    except Exception as e:
        log("IGDB fanart artworks query error: %s" % e, xbmc.LOGERROR)
    ...
    try:
        ...
        log("IGDB fanart screenshot query error: %s" % e, xbmc.LOGERROR)
...
def create_synopsis_xml(out_path, info, region=None):
    ...
    if esrb:
        m = re.match(r"([A-Z])\s*-\s*(.+)", esrb)
        if m:
            letter, name_full = m.group(1), m.group(2)

            if addon.getSetting("rating_letter") == "true" and addon.getSetting("rating_name") != "true":
                esrb = letter
            elif addon.getSetting("rating_name") == "true" and addon.getSetting("rating_letter") != "true":
                esrb = name_full
            else:
                esrb = "%s - %s" % (letter, name_full)
    ...
    if token and name:
        ...
        if ig:
            ...
            if artwork_enabled == "true":
                artwork_dir = os.path.join(os.path.dirname(out_path), "artwork")
                if ig.get("id"):
                    download_igdb_fanart(ig["id"], token, artwork_dir)
